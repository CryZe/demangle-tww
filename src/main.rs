...
fn demangle(function: &str) -> Result<Cow<str>, Cow<'static, str>> {
...
fn main() {
    let file = BufReader::new(File::open("framework.map").unwrap());
    let mut out = BufWriter::new(File::create("result.map").unwrap());
    ...
    for line in file.lines() {
        ...
        let line = regex.replace(&line, |c: &Captures| {
            ...
            format!("{}{}{}",
                    &c[1],
                    demangled.unwrap_or(c[2].into()),//.replace(" ", "").replace("const", " const"),
                    &c[3])
        });
...
fn test_type_parsing() {
    ...
    fn test(input: &str, typ: Type) {
        assert_eq!(parse_type(input).unwrap(), (Some(typ), ""));
    }

    test("v", Normal(false, "void"));
    test("c", Normal(false, "i8"));
    test("s", Normal(false, "i16"));
    test("i", Normal(false, "i32"));
    test("l", Normal(false, "i32"));
    test("f", Normal(false, "f32"));
    test("d", Normal(false, "f64"));
    test("b", Normal(false, "bool"));
    test("6Blargh", Normal(false, "Blargh"));
    test("Fi2hi_3lel",
         Function(false, Box::new(Normal(false, "lel")), vec![Normal(false, "i32"), Normal(false, "hi")]));
    test("A420_Fi_b",
         Array(420,
               Box::new(Function(false, Box::new(Normal(false, "bool")), vec![Normal(false, "i32")]))));
    test("Ps", Pointer(false, Box::new(Normal(false, "i16"))));
    test("Rl", Reference(false, Box::new(Normal(false, "i32"))));
    test("Q35Hello6Blargh4call",
         Path(false, vec!["Hello", "Blargh", "call"]));
}
...
fn test_type_formatting() {
    ...
    fn test(typ: Type, formatted: &str) {
        assert_eq!(typ.to_string(), formatted);
    }

    test(Normal(false, "void"), "void");
    test(Normal(false, "i8"), "i8");
    test(Normal(false, "i16"), "i16");
    test(Normal(false, "i32"), "i32");
    test(Normal(false, "i32"), "i32");
    test(Normal(false, "f32"), "f32");
    test(Normal(false, "f64"), "f64");
    test(Normal(false, "bool"), "bool");
    test(Normal(false, "Blargh"), "Blargh");
    test(Function(false, Box::new(Normal(false, "lel")), vec![Normal(false, "i32"), Normal(false, "hi")]),
         "lel ()(i32, hi)");
    test(Array(420,
               Box::new(Function(false, Box::new(Normal(false, "bool")), vec![Normal(false, "i32")]))),
         "bool ( ()[420])(i32)");
    test(Pointer(false, Box::new(Normal(false, "short"))), "short*");
    test(Reference(false, Box::new(Normal(false, "long"))), "long&");
    test(Path(false, vec!["Hello", "Blargh", "call"]), "Hello::Blargh::call");
}

#[test]
fn test_demangling() {
    fn test(original: &str, expect: &str) {
        assert_eq!(demangle(original).unwrap().to_owned(), expect);
    }

    test("hi", "hi");
    test("hi__Fv", "hi()");
    test("hi__Fv_b", "hi() -> bool");
    test("hi__3FooFv_b", "Foo::hi() -> bool");
    test("hi__Q24Okok3FooFv_b", "Okok::Foo::hi() -> bool");
    test("hi__Q24Okok3FooFPiRs_b", "Okok::Foo::hi(i32*, i16&) -> bool");
}
